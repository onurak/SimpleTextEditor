...
NSString *DocFilenameExtension = @"doc";
...
- (NSMetadataQuery*)textDocumentQuery {
    ...
    if (aQuery) {
        ...
        NSString* filePattern = [NSString stringWithFormat:@"*.%@", DocFilenameExtension];
...
- (IBAction)addDocument:(id)sender {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        newDocumentURL = [newDocumentURL
                          URLByAppendingPathComponent:DocumentsDirectoryName
                          isDirectory:YES];
        newDocumentURL = [newDocumentURL
